...
namespace WebSockets
{
    public class ClientConnection
    {
        ...
        {
            String msg = "Recebi sua msg: " + data;
            var frame = new Frame(Frame.Opcodes.Text, Encoding.UTF8.GetBytes(msg), true);
            Send(frame);
        }
        ...
        private void MandarMsgDoServidor(){
            Thread.Sleep(10000);

            String dados = "E ai, nao vai mandar mais nada ?";

            var frameDados = new Frame(Frame.Opcodes.Text, Encoding.UTF8.GetBytes(dados), true);
            byte[] bufferDados = frameDados.ToBuffer();

            var sendEventArgs = new SocketAsyncEventArgs();
            sendEventArgs.SetBuffer(bufferDados, 0, bufferDados.Length);
